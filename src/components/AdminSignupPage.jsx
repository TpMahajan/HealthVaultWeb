...
const AdminSignupPage = () => {
  ...
  return (
    <motion.div
      ...
      style={{ backgroundImage: "url('/BGMast.png')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}
    >
      {/* Back Button */}
      <motion.button
        ...
        onClick={() => navigate('/')}
        className="group absolute top-8 left-8 flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-all duration-300 font-semibold z-20"
      >
        <div className="bg-white/50 backdrop-blur-sm p-2 rounded-full border border-gray-200 group-hover:bg-blue-50 group-hover:border-blue-200 transition-all duration-300 shadow-sm">
          <ArrowLeft className="w-4 h-4" />
        </div>
      </motion.button>

      <header className="flex items-center justify-between whitespace-nowrap px-4 md:px-10 py-4 relative z-20">
        <div className="flex items-center gap-4 text-[#263238]">
          <div className="h-6 w-6 ml-12"> {/* Add margin to avoid overlap with back button */}
